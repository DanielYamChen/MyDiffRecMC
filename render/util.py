...
def _rgb_to_srgb(f: torch.Tensor, gamma=1.0) -> torch.Tensor:
    if (gamma > 2.0):
...
def _srgb_to_rgb(f: torch.Tensor, gamma=1.0) -> torch.Tensor:
    if (gamma > 2.0):
...
def srgb_to_rgb(f: torch.Tensor) -> torch.Tensor:
    assert (f.shape[-1] == 3 or f.shape[-1] == 4), "number of image channels not correct ..."
...
def save_image(fn, x : np.ndarray) -> np.ndarray:
    ...
    #         imageio.imwrite(fn, np.clip(np.rint(x * 255.0), 0, 255).astype(np.uint8)) # Low compression for faster saving
    #     else:
    ...
    if (os.path.splitext(fn)[1] == ".png"):
        img = np.clip(np.rint(x * 65535.0), 0, 65535).astype(np.uint16)
        
        assert(cv2.imwrite(
            fn,
            cv2.cvtColor(img, cv2.COLOR_RGBA2BGRA),
        )), f"WARNING: FAILED to save image to {fn}"
...
def load_image_raw(fn) -> np.ndarray:
    ...
    return cv2.cvtColor(cv2.imread(fn, cv2.IMREAD_UNCHANGED), cv2.COLOR_BGRA2RGBA)
