...
class DMTetGeometry:
    def __init__(self, grid_res, scale, offset, FLAGS):
        self.FLAGS    = FLAGS
        self.grid_res = grid_res

        tets = np.load('data/tets/{}_tets.npz'.format(self.grid_res))
        self.verts    = torch.tensor(tets['vertices']).float().to('cuda')
        if (isinstance(scale, list) or isinstance(scale, tuple)):
            self.verts[:, 0] *= scale[0]
            self.verts[:, 1] *= scale[1]
            self.verts[:, 2] *= scale[2]
        else:
            self.verts *= scale

        self.verts[:, 0] += offset[0]
        self.verts[:, 1] += offset[1]
        self.verts[:, 2] += offset[2]
    ...
    def tick(self, glctx, batch_targets, lgt, opt_material, loss_fn, iteration, FLAGS, denoiser, phys_cam=None, defocus_net=None):

        ## test
        # print(batch_targets.keys())
        # for key in batch_targets:
        #     if (type(batch_targets[key]) == torch.Tensor):
        #         print(key, ":", batch_targets[key].shape)
        #     elif (type(batch_targets[key]) == list):
